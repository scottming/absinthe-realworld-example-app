defmodule RealWorld.Blog do
  ...
  def find_favorite(%Article{} = article, %User{} = user) do
  ...
  def query(Favorite, %{col: :favorited, current_user: current_user}) do
    from q in Favorite, where: q.user_id == ^current_user.id
  end
  ...
  def run_batch(Favorite, favorite_query, :favorited, article_ids, repo_opts) do
    results =
      from(q in favorite_query, select: {q.article_id, q.id}) |> Repo.all(repo_opts) |> Map.new()

    for id <- article_ids do
      if Map.get(results, id) == nil do
        false
      else
        true
      end
